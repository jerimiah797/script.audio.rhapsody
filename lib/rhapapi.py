...
class Api(object):
    ...
    def log_playstart(self, track_id):
        ...
        data = {"type": "playbackStart", "client": "xbmc_internal",
                "playback": {"id": track_id, "started": ztime, "format": "AAC", "bitrate": 192}}
    ...
    def log_playstop(self, track_id, ztime, duration):
        ...
        url = "%sevents" % (self.S_BASEURL)
        data = {"type": "playbackStop", "duration": duration, "client": "xbmc_internal",
                "playback": {"id": track_id, "started": ztime, "format": "AAC", "bitrate": 192}}
        data = json.dumps(data)
        req = self.__build_member_req(url)
        req.add_header('Content-Type', 'application/json')
    ...
    def get_artist_top_albums(self, artist_id):
        print "Rhapapi: getting top albums from artist"
        url = '%sartists/%s/albums/top?apikey=%s&limit=100&catalog=%s' % (self.BASEURL, artist_id, self.APIKEY, self.app.mem.catalog)
        req = self.__build_req(url)
        results = self.__get_data_from_rhapsody(req, 10)
        if results:
            return results
    ...
    def get_artist_top_tracks(self, artist_id):
        print "Rhapapi: getting top tracks from artist"
        url = '%sartists/%s/tracks/top?apikey=%s&limit=20&catalog=%s' % (self.BASEURL, artist_id, self.APIKEY, self.app.mem.catalog)
        req = self.__build_req(url)
        results = self.__get_data_from_rhapsody(req, 10)
        # container = []
        if results:
    ...
    def get_search_results(self, text, stype):
        ...
        url = "%ssearch?apikey=%s&limit=50&q=%s&type=%s&catalog=%s" % (
            self.BASEURL, self.APIKEY, urllib.quote_plus(text), stype, self.app.mem.catalog)
        req = self.__build_req(url)
        results = self.__get_data_from_rhapsody(req, 10)
        if results:
            return results
